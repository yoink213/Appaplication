...
class AdminOrdersActivity : AppCompatActivity() {
    ...
    private fun executeUpdate(orderId: Int, newPrice: Double, newDeadline: String) {
        lifecycleScope.launch {
            try {
                val map = mapOf("orderId" to orderId, "totalPrice" to newPrice, "deadline" to newDeadline)
                val response = ApiClient.api.updateOrder(map)
                if (response.isSuccessful) {
                    Toast.makeText(this@AdminOrdersActivity, "订单修改成功！", Toast.LENGTH_SHORT).show()
                    loadAllOrders()
                } else {
                    Toast.makeText(this@AdminOrdersActivity, "修改失败：订单已被领取", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                Toast.makeText(this@AdminOrdersActivity, "网络异常", Toast.LENGTH_SHORT).show()
            }
        }
    }
        ...
        lifecycleScope.launch {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    Toast.makeText(this@AdminOrdersActivity, "撤销失败：该单正在运送中", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                Toast.makeText(this@AdminOrdersActivity, "网络异常", Toast.LENGTH_SHORT).show()
            }
